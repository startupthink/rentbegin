...
// เมนู dashboard — เดสก์ท็อปเป็นแถบซ้าย, มือถือเป็นแถบเลื่อนแนวนอน
export default function Sidebar({ variant = 'member', profile, items, active, onSelect }) {
  const isAdmin = variant === 'admin'
  const tabs = items.filter((it) => !it.group)
    ...
    <>
      ...
      <aside className={`side ${isAdmin ? 'side-admin' : ''}`}>
        ...
        <div className="who">
          ...
          <div>
            <div className="who-n">{profile?.name || 'สมาชิก'}</div>
            <div className="who-r">
              {profile?.roleLabel}
      ...
      {/* ---------- มือถือ / แท็บเล็ต ---------- */}
      <div className={`mnav ${isAdmin ? 'mnav-admin' : ''}`}>
        <div className="mnav-scroll">
          {tabs.map((it) => (
            <button
              key={it.id}
              className={`mnav-b ${it.id === active ? 'on' : ''}`}
              onClick={() => onSelect?.(it.id)}
            >
              <span className="i">{it.icon}</span>
